window.onload = () => {
	let str = document.addEventListener('input[type=hidden]').value // value에 값이 들어있는 input태그를 불러온다
	...
	document.addEventListener("keydown", (e) => { // 키다운 이벤트 감지
		if(e.key === ' ' && str.length > cnt) { // 입력한 키가 ' '거나 cnt가 리스트의 길이보다 작으면
			...
			$('.profile img').src = "resources/image/" + text[0];
			$('.profile p').innerText = text[1]; 
			$('.text p').innerText = text[2];
			cnt++;
		}
	});
}